...
function createVisualization() {
    ...
    svg.selectAll("dot")
        ...
        .attr("cy", function (d) {
            return yScale(d.ionizationEnergy);
        })
        .attr("r", 5)
        .style("fill", "#A9CCE3")
        .on("mouseover", function (d) {
            d3.select(this).attr("r", 10).style("fill", "#EC7D7D");
            return tooltip.style("visibility", "visible").text(d.name + "(" + d.atomicNumber + ")" + d.ionizationEnergy).style('color', 'red').style('font-weight', 'bold').style('font-family', 'Oxygen');
        })
        .on("mousemove", function (d) {
            d3.select(this).attr("r", 10).style("fill", "#EC7D7D");
            return tooltip.style("top", (event.pageY - 10) + "px").style("left", (event.pageX + 10) + "px").text(d.name + "(" + d.atomicNumber + "): " + d.ionizationEnergy).style('color', '#898A85').style('font-weight', 'bold').style('font-size', '14px').style('background-color', 'white').style('font-family', 'Oxygen');
        })
    ...
    svg.append("path")
        ...
        .attr("d", d3.line()
            ...
            .y(function (d) { return yScale(d.ionizationEnergy) })
        )
